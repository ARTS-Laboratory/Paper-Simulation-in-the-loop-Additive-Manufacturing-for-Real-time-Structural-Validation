# -*- coding: utf-8 -*-
"""
Created on Mon Apr  3 15:32:12 2023
...
def getMaxMises(odbName,elsetName):
    ...
    if(isStressPresent):
        print ('Maximum von Mises stress %s is %f in element %d'%(region, maxMises, maxElem))
...
if __name__ == '__main__':
    ...
    if not (odbName):
        ...
        print (__doc__)
        exit(1)
    region, maxMises, maxElem = getMaxMises(odbName,elsetName)
    
from abaqus import getWarningReply, YES, NO
import sys
from abaqus import *
if maxMises >= 45:
    reply = getWarningReply(message='Product structural quality is damaged, okay to continue?', buttons=(YES,NO))
    if reply == YES:
        print ('YES clicked')
    elif reply == NO:
        print ('NO clicked')
